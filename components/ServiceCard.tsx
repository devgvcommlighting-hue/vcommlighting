...
export default function ServiceCard({ icon: Icon, title, description, linkHref }: ServiceCardProps) {
    return (
        <div className="flex flex-col items-center text-center p-6 sm:p-8 bg-white/5 backdrop-blur-md border border-white/10 transition-all duration-300 hover:shadow-xl rounded-lg hover:bg-white/10">
            ...
            <Link
                href={linkHref}
                className="text-xl sm:text-2xl font-semibold mb-4 text-gray-500 hover:text-green-600 transition-colors"
            >
                {title}
            </Link>

            {/* Description */}
            <p className="text-sm text-gray-400 mb-6 flex-grow min-h-[100px] sm:min-h-[120px]">
                {description}
            </p>
